...
extension UIView {
    ...
    func showMessage(msg:String? = nil) {
        
        if msg == nil {
            viewWithTag(tagMsg)?.removeFromSuperview()
            return
        }
        
        guard let label = viewWithTag(tagMsg) as? UILabel else {
            let msgLabel = UILabel(frame: .zero)
            msgLabel.tag = tagMsg
            ...
            msgLabel.centerYAnchor.constraint(equalTo: self.centerYAnchor, constant: -50).isActive = true
            return
        }
